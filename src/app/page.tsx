...
function HomeContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.requiresTwoFactor) {
        console.log("Main page: 2FA is required for user:", email);
  ...
  // But make an exception for when 2FA verification is needed
  if ((loading || user) && !showTwoFactor) {
    return (
  ...
  const handleTwoFactorSuccess = (user: any) => {
    ...
    console.log("Main page: 2FA verification successful, user:", user);
